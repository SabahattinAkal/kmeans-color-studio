...
const body=new FormData();body.append('file',f);const k=document.querySelector('#colors').value;
const r=await fetch('/api/quantize?colors='+k,{method:'POST',body});const d=await r.json();if(!r.ok)return alert(d.detail);
...
async def quantize(
    ...
    result = quantize_image(rgb, clusters=colors)
